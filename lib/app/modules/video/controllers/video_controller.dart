...
class VideoController extends GetxController {
  ...
  Future<void> fetchVideo(int? videoId) async {
    try {
      ...
      final res = await BaseClient.get(api: "${EndPoint.myCourseVideos}?videoId=$videoId&userId=$userId");
  ...
  Future<void> getQuizResult({int? courseId, int? videoId}) async {
    final userId = LocalStorageService.getUserId();
    try {
      isLoading.value = true;
      final res = await BaseClient.get(
        api: "https://uttamgyanapi.veteransoftwares.com/api/QuizResponse/userresult?userId=$userId&courseId=$courseId",
      );
